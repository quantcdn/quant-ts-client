...
export enum SSHAccessApiApiKeys {
    headerAuth,
}

export class SSHAccessApi {
    ...
    public setApiKey(key: SSHAccessApiApiKeys, value: string) {
        (this.authentications as any)[SSHAccessApiApiKeys[key]].apiKey = value;
    }
    ...
    public async getSshAccessCredentials (organisation: string, application: string, environment: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: GetSshAccessCredentials200Response;  }> {
        const localVarPath = this.basePath + '/api/v3/organisations/{organisation}/applications/{application}/environments/{environment}/ssh-access'
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: GetSshAccessCredentials200Response;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "GetSshAccessCredentials200Response");
